...
vars_path="${K9S_NEO_VARS_PATH:-${K9S_NEO_REPO_ROOT}/hack/bench/vars.local.json}"
label="${K9S_NEO_BENCH_LABEL:-step6-required-$(date +%Y%m%d-%H%M%S)}"
cold_runs="${K9S_NEO_COLD_RUNS:-1}"
warm_runs="${K9S_NEO_WARM_RUNS:-1}"
ensure_cluster="${K9S_NEO_ENSURE_CLUSTER:-1}"
write_note_path="${K9S_NEO_WRITE_BASELINE_NOTE:-}"

usage() {
  cat <<EOF
usage: $(basename "$0") [--label LABEL] [--cold-runs N] [--warm-runs N] [--bin PATH] [--vars PATH] [--write-note PATH] [--no-ensure-cluster]

Runs the required Step 6 scenario set:
  - pods_startup
  - pods_filter_settle
  - nodes_first_render
  - pod_yaml
...
if [[ "${ensure_cluster}" == "1" ]]; then
  "${K9S_NEO_REPO_ROOT}/hack/local-lab/start-cluster.sh"
  "${K9S_NEO_REPO_ROOT}/hack/local-lab/seed-bench.sh"
fi

if [[ ! -f "${vars_path}" ]]; then
  "${K9S_NEO_REPO_ROOT}/hack/local-lab/write-vars.sh" "${vars_path}"
fi
...
run_output="$(
  python3 "${K9S_NEO_REPO_ROOT}/hack/bench/run.py" \
    ...
    --scenario pods_filter_settle \
    --scenario nodes_first_render \
    --scenario pod_yaml \
    --scenario pod_describe
)"
...
if [[ -n "${write_note_path}" ]]; then
  "${K9S_NEO_REPO_ROOT}/hack/local-lab/write-baseline-note.sh" "${artifact_root}" "${write_note_path}"
fi
