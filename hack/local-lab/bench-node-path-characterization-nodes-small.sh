...
if [[ -n "${write_note_path}" ]]; then
  "${K9S_NEO_REPO_ROOT}/hack/local-lab/write-step7e-node-path-characterization-note.sh" "${artifact_root}" "${write_note_path}"
fi
